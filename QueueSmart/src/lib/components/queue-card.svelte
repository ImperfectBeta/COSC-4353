...
{#if queue}
	<div
		class={clsx(
			...
			entry?.position === 1 && "border-3 border-primary",
		)}
	>
		...
		<div class="flex items-center gap-2 text-sm mt-auto">
			<div class="flex flex-col bg-accent p-1.5 rounded-sm">
				...
				<span class="font-semibold"
					>~{entry?.position === 1 ? 0 : queue.averageWaitTime} min</span
				>
			</div>
			<div class="flex flex-col bg-accent p-1.5 rounded-sm">
			...
			{#if entry}
				{#if entry.position === 1}
					<div
						class="flex flex-col bg-primary/15 border border-primary/30 p-1.5 rounded-sm relative overflow-hidden"
					>
						<div class="absolute inset-0 bg-primary"></div>
						<span
							class="opacity-100 text-xs font-bold text-accent relative z-10 flex items-center gap-1"
						>
							<CircleCheckBigIcon size={12} />
							Your Turn!
						</span>
						<span
							class="font-semibold text-foreground relative z-10"
						>
							Ready now
						</span>
					</div>
		...
		<div
			class="mt-4 pt-4 border-t border-muted-foreground/15 flex justify-end"
		>
			{#if section === "active" && entry}
				<div class="flex items-center gap-2">
					...
					<Button
						...
						onclick={() => onviewticket?.(entry.id.toString())}
					>
			...
			{:else}
				<Button
					variant={queue.status === "open" && !entry
						? "outline"
						: "ghost"}
					size="sm"
	...
	{#if showLeaveModal || showJoinModal || actionError}
		...
		>
			<div
				class={clsx(
					"rounded-xl font-nunito bg-muted bg-card text-muted-foreground shadow-lg shadow-accent/50 p-6 flex flex-col gap-4 w-full max-w-sm animate-in fade-in zoom-in-95 duration-200",
				)}
			>
				{#if showLeaveModal}
					<div class="flex flex-col items-center text-center gap-2">
						...
						<h3
							class="font-semibold leading-none text-lg tracking-tight"
						>
							Leave Queue?
						</h3>
						<p class="text-sm opacity-80">
							Are you sure you want to leave the queue for <strong
								>{queue.name}</strong
							>?
						</p>
					</div>
				...
				{:else if showJoinModal}
					<div class="flex flex-col items-center text-center gap-2">
						...
						<p class="text-sm opacity-80">
							You are now in line for <strong>{queue.name}</strong
							>.
						</p>

						<div
							class="flex flex-col bg-accent p-2 rounded-sm w-full mt-2 text-center"
						>
							<span class="opacity-85 text-xs"
								>Estimated Wait Time</span
							>
							<span class="font-semibold text-lg"
								>{joinWaitTime} minutes</span
							>
						</div>
					</div>

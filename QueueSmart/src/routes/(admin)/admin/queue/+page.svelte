<script lang="ts">
	...
	$: svc = $services ?? [];
	$: qmap = ($queues ?? {}) as Record<string, QueueUser[]>;

	$: selectedService = svc.find(s => s.id === selectedServiceId);
	$: currentQueue =
		selectedServiceId && selectedServiceId in qmap
			? (qmap[selectedServiceId] ?? [])
			: [];

	function serveNext() {
		if (selectedServiceId === null || currentQueue.length === 0) return;
		queues.update(q => {
			const id = selectedServiceId as string;
	...
	function removeUser(userId: number) {
		...
		queues.update(q => ({
			...q,
			[id]: ((q as any)[id] ?? []).filter(
				(u: QueueUser) => u.id !== userId,
			),
		}));
		services.update(list =>
			(list || []).map(s =>
...
<div class="p-8 space-y-8">
	...
	<div class="flex gap-3 flex-wrap">
		{#each $services as service}
			<button
				...
				class="px-4 py-2 rounded-lg cursor-pointer text-sm"
				style={selectedServiceId === service.id
					...
					: "border:1px solid transparent; color:var(--color-background)"}
			>
				{service.name}
				<span class="ml-2 text-xs opacity-70"
					>{service.queueLength}</span
				>
			</button>
		{/each}
	</div>
	...
	{#if selectedService}
		...
		<div
			...
			style="background:var(--color-muted); border:1px solid rgba(0,0,0,0.12)"
		>
			<div
				class="px-6 py-4"
				style="border-bottom:1px solid rgba(0,0,0,0.12); display:flex; align-items:center; justify-content:space-between"
			>
				<div>
					...
					<p class="text-navy-600 text-xs mt-0.5">
						{currentQueue.length} user{currentQueue.length !== 1
							? "s"
							: ""} waiting
					</p>
				</div>
				<button
					...
					class="px-4 py-2 text-sm cursor-pointer rounded-lg font-semibold"
					style={currentQueue.length > 0
						? "background:var(--color-primary); color:var(--color-foreground)"
						: "background:rgba(0,0,0,0.06); color:var(--color-background); cursor:not-allowed"}
				>
					Serve Next ›
				</button>
			</div>
				...
				<ul>
					{#each currentQueue as user, i (user.id)}
						<li
							...
							style="list-style:none; display:flex; align-items:center; gap:1rem; padding:1rem 1.5rem"
						>
							<!-- Position badge -->
							<span
	...
	{:else}
		<div
			class="bg-zinc-900 border border-zinc-800 rounded-xl px-6 py-12 text-center text-zinc-600 text-sm"
		>
